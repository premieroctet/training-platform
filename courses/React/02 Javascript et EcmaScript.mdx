...
- L'écosystème React est **récent** et adopte **les dernières syntaxes JS**
- La maîtrise d'**ES6** est essentielle
...
- **ECMAScript (ES)**: Spécification gérée par ECMA, normalisant à la fois le
...
- On oublie **var** 👋 et on utilise :

  - **const** pour empêcher les changements de valeur ;
  - **let** pour réduire son scope au bloc actuel ;
...
- Souvent utilisés avec React :

  - Modifiables même si c'est déclaré via une constante, pourquoi ? 🤔
...
- **forEach** permet de boucler sur un tableau
  ...
  - Passe en paramètre la valeur et l'index
...
- On utilise **map** pour créer un nouveau tableau à partir d'un autre

---

<SlideHeader slideTitle="Array : concat, filter" />

#### Autres méthodes : concat, filter

- **concat** créé un nouveau tableau (contrairement à push)
- **filter** retourne un nouveau tableau filtré sur la condition donnée :
...
- Ancienne façon de déclarer un composant React
...
<SlideHeader slideTitle="Spread / Déstructuration" />

#### Spread Operator & Déstructuration ✨

- Affecter par déstructuration (très utilisé avec React):
...
- Spread opérateur (objet et tableau) :
...
<SlideHeader slideTitle="Paramètres par défaut / templates" />

#### Paramètres par défaut & templates

- Meilleure gestion de la concaténation (by bye les + 👋
...
- Syntaxe courte déclarant des closures liées au contexte courant
...
- Pour réduire la complexité des appels asynchrones, ES6 introduit un nouveau
...
- Implémentation + complexe, utilisation aisée
...
- L'instruction **import** est utilisée pour importer des fonctions, des objets
  ou des valeurs primitives exportées depuis un module externe (remplace le
  **require()**).
